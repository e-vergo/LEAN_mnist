...
This file contains properties of dense layers and their compositions, organized into:
- **Dimension Consistency**: Type-level guarantees via dependent types
- **Linearity Properties**: Affine transformation properties of dense layers
- **Composition Theorems**: Properties preserved under layer composition

## Implementation Notes

Dimension correctness is enforced by Lean's dependent type system. If code type-checks
with `Vector n` or `Matrix m n`, the dimensions are guaranteed at compile time.

## Verification Status

- Type-level dimension safety: Enforced by dependent types ✓
- Linearity properties: Partial (requires `matvec_linear` from Core.LinearAlgebra)
- Differentiability theorems: Planned (requires SciLean integration)
- Composition theorems: Proven by construction where applicable ✓
-/
