...
- ReLU uses `if` which may have limitations for AD at discontinuity (x = 0)
- Softmax currently lacks numerical stability (log-sum-exp trick) - use with caution
...
**Known Limitations:**
- ReLU gradient undefined at x=0 (convention: use 0 or 1)
...
/-- ReLU activation function: max(0, x)
    ...
    **Gradient at x=0:** By convention, we use 0 (subgradient).
...
/-- Softmax activation function: exp(x[i]) / Σⱼ exp(x[j])
    ...
    **CRITICAL:** Currently lacks numerical stability (log-sum-exp trick).
    ...
    TODO: Add differentiation properties
    Issue: Softmax is critical for classification but numerically unstable -/
@[inline]
def softmax {n : Nat} (x : Vector n) : Vector n :=
  -- Simple implementation without numerical stability trick
  -- FIXME: Add max subtraction for stability
...
-- ========================================
-- These are separate from the AD system and used for numerical validation.
